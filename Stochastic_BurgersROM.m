...
Dir= ['/home/xupingxy/Documents/testcode/FEM_POD_codes_to_David/Data/'];
fileID=['Burgers1D_FEM_00_P1_h9p8en04_dt1p0en04_cta1_dns.mat'];
filename=[Dir,fileID];

fprintf('load data \n')
load (filename,'x','Mass','S','u_save','dW','domain','e_conn');
fprintf('Complete ! Running')
...
podopt.p=Tr;
[pod_u,CumEng,CumEng_ratio,POD_all,Diag_S,lp]=POD_basis(x,u_save,Mass,S,podopt);             %-------generate POD basis;   
r=podopt.p;
...
%         Nh=Nh+Nonlin0*Ones*C_bar;
        ...
        %Nh=C_bar'*Nonlin
        F=Mr*C0-opt.nu*opt.del*(Sr*C0)-opt.del*Nh-opt.del*sigma*dW(k)*Mr;       %---F vector
